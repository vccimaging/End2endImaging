...
In commercial software like Zemax, diffractive surfaces are typically simulated by adding a ray bending angle to the standard refracted ray. In End2endImaging, phase surfaces operate on the same principle using ray optics. (End2endImaging also supports diffractive surfaces simulated via wave optics; please refer to the `end2end_imaging/deeplens/diffractive_surface/` directory. Both modules represent diffractive surfaces, differing primarily in the simulation method.) Diffraction pattern can also be applied on curved surfaces, which has not been implemented yet.
